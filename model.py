...
class ColumnParallelLinear(Linear):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = False, dtype = None, comm_group=None):
        ...
        if comm_group is None:
            ...
            assert out_features % comm_group.size() == 0, f"Output features must be divisible by comm group size (comm_group_size={comm_group.size()})"
            self.part_out_features = out_features // comm_group.size()
...
class RowParallelLinear(Linear):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = False, dtype = None, comm_group=None):
        ...
        if comm_group is None:
            ...
            assert in_features % comm_group.size() == 0, f"Input features must be divisible by comm group size (comm_group_size={comm_group.size()})"
            self.part_in_features = in_features // comm_group.size()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if world_size > 1:
            y = y.float()
            dist.all_reduce(y, group=self.comm_group)
...
class Attention(nn.Module):
    ...
    def __init__(self, layer_id: int, args: ModelArgs):
        ...
        if g_projection_comm_group is None:
            ...
            self.n_local_groups = self.n_groups // g_projection_comm_group.size()
        ...
        if g_projection_comm_group is None:
            ...
            self.wo_a = ColumnParallelLinear(self.n_heads * self.head_dim // self.n_groups, self.n_groups * args.o_lora_rank, dtype=torch.bfloat16, comm_group=g_projection_comm_group)
    ...
    def forward(self, x: torch.Tensor, start_pos: int):
        ...
        if self.changed:
            # concat tensor o of adjacent pair ranks such as [0, 1], [2, 3], etc
            if g_pair_comm_group is None:
                raise RuntimeError(f"Rank {rank}: g_pair_comm_group is None but self.changed=True")
            o_list = [torch.empty_like(o) for _ in range(g_pair_comm_group.size())]
            dist.all_gather(o_list, o, group=g_pair_comm_group)
